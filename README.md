...
When running the Docker native container of `producer-service` and submitting an invalid event according to `javax.validation.constraints`, like the field `titolo` in `CreateRAINewsRequest` must not be blank, for instance
```
http :9080/api/news/rai titolo2="Sciopero della metropolitana di Roma"
```
the response is
```
...
```
The expected response should be the following (as it happens with Docker JVM)
```
